...
const InterimSiteDetails = () => {
        ...
        if (hasInterimSite === 'Yes') {
          router.push({
            pathname: returnToDraft
              ...
              : `${process.env.NX_EXPORT_URL}/interim-site-details`,
            query: { id },
          });
        } else {
          if (existingInterimSite !== null) {
            ...
            router.push({
              pathname: returnToDraft
                ? `${process.env.NX_EXPORT_URL}/submit-an-export-tasklist`
                : `${process.env.NX_EXPORT_URL}/recovery-facility-details`,
              query: { id },
            });
          }
        }
  ...
  const removeExistingInterimSite = (returnToDraft) => {
    try {
      fetch(
        `${process.env.NX_API_GATEWAY_URL}/submissions/${id}/recovery-facility/${existingInterimSite}`,
        {
          method: 'DELETE',
          headers: {
