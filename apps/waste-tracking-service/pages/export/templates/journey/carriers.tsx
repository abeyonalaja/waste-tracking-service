        ...
        .then((data) => {
          if (data !== undefined) {
            ...
            router.push({
              ...
              query: { templateId, carrierId: getEmptyCarrier[0].id },
            });
          }
        });
    } catch (e) {
  ...
  const handleConfirmSubmit = (e: FormEvent) => {
    ...
    if (isNotEmpty(newErrors)) {
      ...
      if (confirm === 'yes') {
        ...
        try {
          fetch(
            `${process.env.NX_API_GATEWAY_URL}/templates/${templateId}/carriers/${item.id}`,
            {
              method: 'DELETE',
              headers: {
                'Content-Type': 'application/json',
                    ...
                    <>
                      ...
                      <GovUK.Paragraph id="item-to-delete">
                        {t('exportJourney.wasteCarrier.carriersPage.hint', {
                          n: 5 - wasteCarriersPage.data.values?.length,
                        })}
                      </GovUK.Paragraph>
