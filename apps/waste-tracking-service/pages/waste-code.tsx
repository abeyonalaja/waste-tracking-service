...
export function WasteCode() {
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      if (id !== undefined) {
        try {
          fetch(`${process.env.NX_API_GATEWAY_URL}/submissions/${id}/waste-description`)
            .then((response) => {
      ...
      if (isNotEmpty(newErrors)) {
        ...
        try {
          fetch(url, {
            ...
            .then((data) => {
              if (data !== undefined) {
                const path = returnToDraft ? "/dashboard/submit-an-export" : "/ewc-code"
                router.push({
                  pathname: path,
                  query: { id }
                });
