...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('single.receiverAddress.metadata');
...
export default async function ReceiverAddressPage({
  params,
  ...
  try {
    response = await fetch(
      `${apiUrl}/ukwm/drafts/${params.id}/receiver-address`,
      {
        method: 'GET',
        cache: 'no-store',
        headers: {
  ...
  const CreateContent = async (key: string) => {
    const t = await getTranslations('single.receiverAddress');
    return (
      <>
  ...
  return (
    <AddressSearch
      ...
      destination="/receiver/contact"
      apiPartial="/receiver-address"
      section="Receiver"
    />
  );
}
